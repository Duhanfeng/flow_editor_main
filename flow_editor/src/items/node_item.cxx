...
void NodeItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* item, QWidget* widget)
{
    ...
    //painter->save();
    ////使用视图的缩放系数来缩放QPainter
    //painter->scale(scale, scale);
    ////启用抗锯齿和平滑像素图转换
    //painter->setRenderHint(QPainter::Antialiasing, true);
    //painter->setRenderHint(QPainter::SmoothPixmapTransform, false);
    //int x = (int)(node_sub_geometry_.bounding_rect.x() / scale);
    //int y = (int)(node_sub_geometry_.bounding_rect.y() / scale);
    //int w = (int)(node_sub_geometry_.bounding_rect.width() / scale);
    //int h = (int)(node_sub_geometry_.bounding_rect.height() / scale);
    //painter->drawPixmap(x, y, w, h, pixmap_cache_);
    //painter->restore();

    paintTo(painter, data_, node_sub_geometry_);
}
void NodeItem::initializePixmap()
{
...
void NodeItem::paintTo(QPainter* painter, const NodeData& data, const NodeSubGeometry& node_sub_geometry)
{
