...
void OutNodePainter::paintFrame(QPainter* painter, OutNodeItem* item)
{
    //画各个区域(调试用)
...
void OutNodePainter::paintNodeRect(QPainter* painter, OutNodeItem* item)
{
    QPen pen;
    ...
    //画主区域
    const auto& components = item->geometry_->components();
    painter->setBrush(item->draft_connection_item_ ? QColor(0x546281) : QColor(0x861305));
    painter->drawPolygon(components.node_polygon);
}
void OutNodePainter::paintCaption(QPainter* painter, OutNodeItem* item)
{
    const auto& components = item->geometry_->components();
...
void OutNodePainter::paintConnectionPoints(QPainter* painter, OutNodeItem* item)
{
    const auto& components = item->geometry_->components();
